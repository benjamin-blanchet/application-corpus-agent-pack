...
| `requirement` | Whether absence blocks the requested corpus work. |
...
emit `visible_unverified`, `not_visible`, `unreachable`, `permission_denied`,
`mapping_missing` or `unsafe_to_probe` for this run. Never write that result to
the source registry, coverage file or global corpus state.
