...
Before using a connected source, read its contract in `doc/_meta/information-sources.yaml` and its local reference, then run `sources/runtime-source-probe`.

If this runtime cannot use the selected transport, do not silently fall back. A
required source blocks unless a structured operator waiver exists; an optional
source may yield explicitly partial discovery. Report the dated observation and
preserve historical evidence in `doc/_meta/source-coverage.yaml`.
